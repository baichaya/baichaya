<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ibms:accessRecord:add']"
              >新增</el-button
            >
          </el-col>
          <el-col :span="1.5">
      ...
      <el-table v-loading="loading" :data="accessRecordList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="刷卡时间" align="center" prop="eventDate" width="180">
          <template #default="scope">
            <span>{{ parseTime(scope.row.eventDate, "{y}-{m}-{d} {H}:{m}:{s}") }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="accessRecordFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="门区名称" prop="areaName">
        ...
        <el-form-item label="刷卡人员证件号" prop="employeeId">
          <el-input v-model="form.employeeId" placeholder="请输入刷卡人员证件号" />
        </el-form-item>
        <el-form-item label="刷卡人员姓名" prop="employeeName">
          <el-input v-model="form.employeeName" placeholder="请输入刷卡人员姓名" />
        </el-form-item>
        <el-form-item label="刷卡人员编号" prop="employeeSysNo">
          <el-input v-model="form.employeeSysNo" placeholder="请输入刷卡人员编号" />
        </el-form-item>
        ...
        <el-form-item label="1带口罩，2未戴，0未知" prop="mask">
          <el-input v-model="form.mask" placeholder="请输入1带口罩，2未戴，0未知" />
        </el-form-item>
...
import { AccessRecordVO, AccessRecordQuery, AccessRecordForm } from "@/api/ibms/sys-entranceGuard/accessRecord/types";
...
const data = reactive<PageData<AccessRecordForm, AccessRecordQuery>>({
  ...
  rules: {
    ...
    areaName: [{ required: true, message: "门区名称不能为空", trigger: "blur" }],
    doorName: [{ required: true, message: "房门名称不能为空", trigger: "blur" }],
    employeeId: [{ required: true, message: "刷卡人员证件号不能为空", trigger: "blur" }],
    employeeName: [{ required: true, message: "刷卡人员姓名不能为空", trigger: "blur" }],
    employeeSysNo: [{ required: true, message: "刷卡人员编号不能为空", trigger: "blur" }],
    eventDate: [{ required: true, message: "刷卡时间不能为空", trigger: "blur" }],
    name: [{ required: true, message: "刷卡人员部门名称不能为空", trigger: "blur" }],
    serial: [{ required: true, message: "刷卡卡号不能为空", trigger: "blur" }],
    stationName: [{ required: true, message: "设备名称不能为空", trigger: "blur" }],
    typedescribe: [{ required: true, message: "卡类型不能为空", trigger: "blur" }],
  },
});
...
const submitForm = () => {
  accessRecordFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        const res = await updateAccessRecord(form.value).finally(() => (buttonLoading.value = false));
        proxy?.$modal.msgSuccess(res.msg);
      } else {
        const res = await addAccessRecord(form.value).finally(() => (buttonLoading.value = false));
        proxy?.$modal.msgSuccess(res.msg);
      }
      dialog.visible = false;
      await getList();
