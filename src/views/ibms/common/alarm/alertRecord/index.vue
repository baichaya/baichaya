      ...
      <div class="search" v-show="showSearch">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
          <el-form-item label="设备id" prop="deciveId">
            <el-input v-model="queryParams.deciveId" placeholder="请输入设备id" clearable style="width: 240px" @keyup.enter="handleQuery" />
          </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ibms:alertRecord:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['ibms:alertRecord:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['ibms:alertRecord:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ibms:alertRecord:export']">导出</el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ibms:alertRecord:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ibms:alertRecord:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
import { listAlertRecord, getAlertRecord, delAlertRecord, addAlertRecord, updateAlertRecord } from '@/api/ibms/common/alarm/alertRecord/index';
import { AlertRecordVO, AlertRecordQuery, AlertRecordForm } from '@/api/ibms/common/alarm/alertRecord/types';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;
const { device_group, device_type,alarm_state } = toRefs<any>(proxy?.useDict('device_group', 'device_type','alarm_state'));
...
const handleDelete = async (row?: AlertRecordVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除告警记录编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('ibms/alertRecord/export', {
    ...queryParams.value
