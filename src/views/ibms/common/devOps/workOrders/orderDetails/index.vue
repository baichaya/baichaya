    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['workOrders:orderDetails:add']"
              >新增</el-button
            >
          </el-col>
          <el-col :span="1.5">
...
const data = reactive<PageData<OrderDetailsForm, OrderDetailsQuery>>({
  ...
  rules: {
    workOrderDetailType: [{ required: true, message: "工单类型不能为空", trigger: "change" }],
    workOrderDetailValue: [{ required: true, message: "工单详细信息值不能为空", trigger: "blur" }],
  },
});
...
const handleDelete = async (row?: OrderDetailsVO) => {
  ...
  await proxy?.$modal
    .confirm('是否确认删除工单管理-工单详细编号为"' + _workOrderDetailSns + '"的数据项？')
