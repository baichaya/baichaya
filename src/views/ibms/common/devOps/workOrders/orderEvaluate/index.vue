  ...
  <div class="p-2">
      ...
      <div class="search" v-show="showSearch">
        ...
        >
          ...
          <el-form-item label="评价等级" prop="evaluateLv">
            <el-select
              v-model="queryParams.evaluateLv"
              placeholder="请选择评价等级"
              clearable
            >
              <el-option
    ...
    >
      ...
      >
        ...
        <el-form-item label="改进建议" prop="evaluateRecommendation">
          <el-input
            v-model="form.evaluateRecommendation"
            placeholder="请输入改进建议"
          />
        </el-form-item>
...
const submitForm = () => {
  orderEvaluateFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await updateOrderEvaluate(form.value).finally(
          () => (buttonLoading.value = false)
        );
      } else {
        await addOrderEvaluate(form.value).finally(
          () => (buttonLoading.value = false)
        );
      }
