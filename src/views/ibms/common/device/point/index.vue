    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ibms:point:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['ibms:point:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['ibms:point:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ibms:point:export']">导出</el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ibms:point:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ibms:point:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
const { device_group, device_type, is_binding } = toRefs<any>(proxy?.useDict('device_group', 'device_type', 'is_binding'));
...
const handleDelete = async (row?: PointVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除设备点位编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delPoint(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('ibms/point/export', {
    ...queryParams.value
