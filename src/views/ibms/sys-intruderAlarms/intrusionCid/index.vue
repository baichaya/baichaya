<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="intrusionCidList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="intrusionCidFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="CID码" prop="alarmCode">
...
import { IntrusionCidVO, IntrusionCidQuery, IntrusionCidForm } from "@/api/ibms/sys-intruderAlarms/intrusionCid/types";
...
const data = reactive<PageData<IntrusionCidForm, IntrusionCidQuery>>({
  ...
  rules: {
    ...
    messagePush: [{ required: true, message: "消息推送不能为空", trigger: "blur" }],
  },
});
  ...
  intrusionCidFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.alarmCode) {
        const res = await updateIntrusionCid(form.value).finally(() => (buttonLoading.value = false));
        proxy?.$modal.msgSuccess(res.msg);
      } else {
        const res = await addIntrusionCid(form.value).finally(() => (buttonLoading.value = false));
        proxy?.$modal.msgSuccess(res.msg);
      }
      dialog.visible = false;
      await getList();
