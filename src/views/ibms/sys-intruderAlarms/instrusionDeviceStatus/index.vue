        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              icon="Plus"
              @click="handleAdd"
              v-hasPermi="['intrusionAlarm:instrusionDeviceStatus:add']"
              >新增</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['intrusionAlarm:instrusionDeviceStatus:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['intrusionAlarm:instrusionDeviceStatus:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              icon="Download"
              @click="handleExport"
              v-hasPermi="['intrusionAlarm:instrusionDeviceStatus:export']"
              >导出</el-button
            >
          </el-col>
...
<script setup name="InstrusionDeviceStatus" lang="ts">
import {
  listInstrusionDeviceStatus,
  getInstrusionDeviceStatus,
  delInstrusionDeviceStatus,
  addInstrusionDeviceStatus,
  updateInstrusionDeviceStatus,
} from "@/api/ibms/sys-intruderAlarms/instrusionDeviceStatus";
import {
  InstrusionDeviceStatusVO,
  InstrusionDeviceStatusQuery,
  InstrusionDeviceStatusForm,
} from "@/api/ibms/sys-intruderAlarms/instrusionDeviceStatus/types";

const { proxy } = getCurrentInstance() as ComponentInternalInstance;

const instrusionDeviceStatusList = ref<InstrusionDeviceStatusVO[]>([]);
...
const instrusionDeviceStatusFormRef = ref<ElFormInstance>();
...
const initFormData: InstrusionDeviceStatusForm = {
  id: undefined,
  deviceId: undefined,
...
const getList = async () => {
  loading.value = true;
  const res = await listInstrusionDeviceStatus(queryParams.value);
  instrusionDeviceStatusList.value = res.rows;
...
const reset = () => {
  ...
  instrusionDeviceStatusFormRef.value?.resetFields();
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: InstrusionDeviceStatusVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加设备或通道的状态变化日志";
};

/** 修改按钮操作 */
const handleUpdate = async (row?: InstrusionDeviceStatusVO) => {
  reset();
  const _id = row?.id || ids.value[0];
  const res = await getInstrusionDeviceStatus(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改设备或通道的状态变化日志";
};

/** 提交按钮 */
const submitForm = () => {
  instrusionDeviceStatusFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
...
const handleExport = () => {
  proxy?.download(
    "intrusionAlarm/instrusionDeviceStatus/export",
    {
      ...queryParams.value,
    },
    `instrusionDeviceStatus_${new Date().getTime()}.xlsx`
  );
};
